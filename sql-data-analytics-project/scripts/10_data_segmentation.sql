...
-- Segment customers based on spending behavior and history
WITH customer_spending AS (
    SELECT
        ...
        EXTRACT(YEAR FROM AGE(MAX(order_date), MIN(order_date)))*12 + EXTRACT(MONTH FROM AGE(MAX(order_date), MIN(order_date))) AS lifespan_months
